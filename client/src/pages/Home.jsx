...
const Home = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      setLoading(true);
      try {
        const response = await fetch('https://imagenation-backend.vercel.app/api/v1/posts', {
          method: 'POST',
          headers: {
  ...
  const generateImage = async () => {
    if (form.prompt && form.api_key) {
      try {
        ...
        const response = await fetch('https://imagenation-backend.vercel.app/api/v1/dalle', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
